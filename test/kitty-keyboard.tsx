...
test('kitty protocol - backspace release', t => {
...
test('kitty protocol - Ctrl letters retain their unshifted key code', t => {
...
test('kitty protocol - Ctrl letters preserve their input character', t => {
...
test('kitty protocol - modifier-only key (57441 leftshift) is non-printable', t => {
	const result = parseKeypress('\u001B[57441;2u');
...
test('kitty protocol - modifier-only key (57442 leftcontrol) is non-printable', t => {
	const result = parseKeypress('\u001B[57442;5u');
...
test.serial(
	...
	async t => {
		...
		await delay(30);
		t.true(app.writtenStrings.includes('\u001B[>1u'));
...
test.serial(
	...
	async t => {
		...
		app.stdin.push(textEncoder.encode('\u001B[?1u'));
		await delay(30);
		t.true(app.writtenStrings.includes('\u001B[>1u'));
		t.deepEqual(app.inputs, []);
	},
...
test.serial(
	'kitty protocol - auto detection handles split responses with surrounding input',
	async t => {
		const app = createDetectionApp();
		t.teardown(app.unmount);
		app.stdin.push('before\u001B[?');
		await delay(1);
		app.stdin.push('0uafter');
		await delay(30);
		t.true(app.writtenStrings.includes('\u001B[>1u'));
		t.deepEqual(app.inputs, ['before', 'after']);
	},
);
