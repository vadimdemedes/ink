...
//     nudge); if they coalesced into a single render the off-by-one would
//     never be planted. Each phase yields a macrotask (setTimeout 0) so
//     React's already-scheduled work commits before the next update, then
//     awaits `waitUntilRenderFlush()` so the frame's bytes reach stdout before
//     the next phase starts. The flush alone is not enough: without the
//     macrotask hop all three updates batch into a single frame and the test
//     passes without exercising the bug. The test's distinct-frame checks and
//     its "LIVE-0" positive control guard against this; the control keys on
//     the lowercase/uppercase `live`/`LIVE` distinction, so it must stay
//     case-sensitive.
